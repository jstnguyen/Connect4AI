...
class connect4():
	def __init__(self, player1, player2, board_shape=(6,7), visualize=False, game=0, save=False,
		...
		self.topPosition = (np.ones(board_shape[1]) * (board_shape[0]-1)).astype('int32') # what row will it go into, 0 is top 5 is bottom, decrement by 1 when drop in
	...
	def play(self):
		...
		if self.save:
			self.saveGame()
		if self.verbose:
			if len(self.histor0y[0]) + len(self.history[1]) == self.shape[0] * self.shape[1]:
	...
	def gameOver(self, j, player):
		...
		minLeftDiag = [max(j - 3, j), min(i + 3, self.shape[0]-1)] # instead of checking entire array, checks 4 possible wins from last move
